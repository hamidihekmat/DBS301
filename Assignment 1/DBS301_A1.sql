...
SELECT SUBSTR(EMPLOYEE_ID,1,3) "ID", SUBSTR((LAST_NAME || ', ' || FIRST_NAME), 1, 25) "Full Name", JOB_ID "Job", '[' || TO_CHAR(HIRE_DATE, 'FMMONTH') || ' ' || TO_CHAR(LAST_DAY(HIRE_DATE), 'DDth') || ' of ' || 
...
(((T2.SALARY + 1000) * 12) * (1 + NVL(T2.COMMISSION_PCT,0))) "Total Income" FROM EMPLOYEES T1
...
SELECT SUBSTR(DEPARTMENT_NAME || ' '|| NVL(CITY,'Not Assigned'),1,25) "City", COUNT(T1.JOB_ID) "# of Jobs" FROM EMPLOYEES T1 
LEFT JOIN DEPARTMENTS T2 USING (DEPARTMENT_ID)
FULL JOIN LOCATIONS T3 USING (LOCATION_ID) 
